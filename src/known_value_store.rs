...
///
/// ```
/// use known_values::{KnownValuesStore, KnownValue};
/// use std::collections::HashMap;
///
...
impl KnownValuesStore {
    ...
    /// ```
    pub fn new<T>(known_values: T) -> Self where T: IntoIterator<Item = KnownValue> {
    ...
    pub fn known_value_for_name(name: &str, known_values: Option<&Self>) -> Option<KnownValue> {
        known_values.and_then(|known_values| known_values.known_value_named(name)).cloned()
    }
